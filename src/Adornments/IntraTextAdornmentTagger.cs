        ...
        where TAdornment : UIElement
    {
        ...
        private void AsyncUpdate()
        {
            ...
            if (snapshot != buffer.CurrentSnapshot)
            {
                ...
                foreach (KeyValuePair<SnapshotSpan, TAdornment> keyValuePair in _adornmentCache)
                {
        ...
        public virtual IEnumerable<ITagSpan<IntraTextAdornmentTag>> GetTags(NormalizedSnapshotSpanCollection spans)
        {
            ...
            // Grab the adornments.
            IEnumerator<TagSpan<IntraTextAdornmentTag>> tags = GetAdornmentTagsOnSnapshot(translatedSpans).GetEnumerator();

            while (tags.MoveNext())
            {
                TagSpan<IntraTextAdornmentTag> current = tags.Current;
                SnapshotSpan span = current.Span.TranslateTo(requestedSnapshot, SpanTrackingMode.EdgeExclusive);
                var tag = new IntraTextAdornmentTag(current.Tag.Adornment, current.Tag.RemovalCallback, current.Tag.Affinity);

...
namespace EditorColorPreview
{
    ...
    internal sealed class ColorAdornmentTaggerProvider2 : IViewTaggerProvider
    {
        public ITagger<T> CreateTagger<T>(ITextView textView, ITextBuffer buffer) where T : ITag =>
            buffer.Properties.GetOrCreateSingletonProperty(() => new ColorAdornmentTagger2(buffer)) as ITagger<T>;
    }

    internal class ColorAdornmentTagger2 : ITagger<IntraTextAdornmentTag>, IDisposable
    {
        ...
        public ColorAdornmentTagger2(ITextBuffer buffer)
        {
            ...
            foreach (SnapshotSpan span in spans.Where(s => !s.IsEmpty && s.Length >= 3))
            {
                ...
                foreach (Match match in _regex.Matches(text))
                {
                    ...
                    if (color != Color.Empty)
                    {
                        ...
                        IntraTextAdornmentTag tag = new(new ColorAdornment(winColor), null, PositionAffinity.Successor);
